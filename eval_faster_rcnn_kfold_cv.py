        ...
        per_class_summary.append(
            {
                ...
                "total_ground_truth_boxes": sum(
                    int(required_float(record, "num_gt", "per-class metric"))
                    for record in class_results
                ),
                "AP50": numeric_summary(
                    [required_float(record, "ap50", "per-class metric") for record in class_results]
                ),
                "AP50_95": numeric_summary(
                    [required_float(record, "ap50_95", "per-class metric") for record in class_results]
                ),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate any completed local Faster R-CNN K-fold study and aggregate overall/per-class metrics. "
    ...
    parser.add_argument("--run-root", type=Path, required=True, help="Run root produced by run_faster_rcnn_kfold_cv.py")
    parser.add_argument(
        "--checkpoint-template",
        type=str,
...
def main() -> None:
    ...
    fold_roots, dataset_config = validate_cv_layout(
        args.data_root,
        folds,
    ...
    args.output = (
        ...
        / (
            f"cv_evaluation_{args.split}_per_class_nms"
    ...
    for position, fold in enumerate(folds, start=1):
        ...
        metrics_output = checkpoint.parent / (
            f"{args.split}_metrics_per_class_nms"
            f"{'_resize_' + args.resize_mode if args.resize_mode is not None else ''}"
            f"{'_tta_hflip' if args.tta_hflip else ''}_conf_{confidence_tag(args.conf_thresh)}.json"
        )
    ...
    summary = {
        "purpose": "Generic custom Faster R-CNN K-fold cross-validation evaluation",
