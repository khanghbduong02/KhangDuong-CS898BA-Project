...
>
> **Last updated:** 2026-09-24 — Phase 2 Component 1 (train-only Ultralytics-aligned HSV jitter) implemented and tested in both custom dataset loaders; portable full 3-fold training/validation commands are documented. The controlled letterbox+HSV experiment is ready to run; no new training result is claimed yet.
...
- **Post-submission practical study:** future transfer-learning and Ultralytics-style engineering work is separate from the submitted strict-scratch study. [POST_SUBMISSION_PRACTICAL_PLAN.md](POST_SUBMISSION_PRACTICAL_PLAN.md) freezes the current dataset, labels, grouped folds, class taxonomy, preprocessing inputs, and candidate public-test boundary. Phase 1 (shared letterbox geometry full CV) is completed and adopted; Phase 2 (controlled train-only augmentation) is now active. All resulting metrics remain selection-aligned grouped-development evidence, not independent generalization claims.
- **Post-submission shared letterbox geometry: completed, validated, and adopted.** [image_geometry.py](image_geometry.py) provides a reversible source-to-model transform for historical `stretch` and aspect-ratio-preserving `letterbox` modes across training, evaluation, CV orchestration, and demos. Full 3-fold grouped cross-validation at 960 px (`runs/yolo26/post_submission_letterbox` and `runs/faster_rcnn/post_submission_letterbox`) confirmed:
  - Custom YOLO26 (scale `n`): mAP50 improved from submitted stretch $0.1586 \pm 0.0104 \rightarrow \mathbf{0.1985 \pm 0.0063}$ (+25.2% relative gain), mAP50-95 from $0.0512 \pm 0.0043 \rightarrow \mathbf{0.0691 \pm 0.0053}$, precision from $0.1106 \pm 0.0060 \rightarrow 0.1137 \pm 0.0073$, and recall from $0.1306 \pm 0.0067 \rightarrow 0.1453 \pm 0.0139$ across selected epochs 49 / 45 / 45. Over extrusion AP50 rose to $0.3432$ and Warping to $0.3763$.
  - Custom Faster R-CNN (scale `s`): mAP50 maintained solid parity with submitted scheduled stretch ($0.3258 \pm 0.0275 \rightarrow \mathbf{0.3280 \pm 0.0282}$), mAP50-95 was $0.1198 \pm 0.0097$ (vs $0.1239 \pm 0.0111$), precision was $0.2985 \pm 0.1086$ (vs $0.2960$), and recall was $0.1167 \pm 0.0183$ (vs $0.1146$) across selected epochs 28 / 42 / 50.
  - Decision: Letterbox is adopted as the standardized geometry for the practical track. Historical scratch stretch runs remain frozen.
- **Phase 2 Component 1 implementation (ready for controlled CV):** [online_augmentation.py](online_augmentation.py) adds the `hsv` in-memory policy using official Ultralytics default gains (`h=0.015`, `s=0.70`, `v=0.40`). Both custom trainers apply the selected online policy only to the training dataset; validation uses `none`. The transform preserves image geometry, bounding boxes, labels, source files, and class counts. Deterministic seed reproducibility, RGB bounds, and target invariance tests are in [tests/test_online_augmentation.py](tests/test_online_augmentation.py). K-fold runners pass and checkpoint the policy. This is a completed code/test foundation only; the new full 3-fold training and validation results remain pending.
- **Deferred data-governance groundwork:** `prepare_post_submission_data_inventory.py` and `screen_post_submission_holdout.py` remain available for a future separate data-governance study. Their completed inventory of `3,421` records and `823` development groups remains documentation-only in the frozen-dataset practical study. Do not use their review, corrected-data, source-collection, or holdout paths to modify this study's data, labels, folds, or claims.
...
- **Post-submission letterbox full cross-validation: completed and adopted for the practical track (2026-08-24).** Both local models completed full three-fold grouped cross-validation at 960 px under `runs/yolo26/post_submission_letterbox` and `runs/faster_rcnn/post_submission_letterbox` with `resize_mode=letterbox`, preserving all other selected hyperparameters, loss gains, positive weighting (`power=0.25`), and seed `42`:
  - Custom YOLO26 (scale `n`): selected epochs `49` / `45` / `45` with aggregate mAP50 $0.1985 \pm 0.0063$, mAP50-95 $0.0691 \pm 0.0053$, precision $0.1137 \pm 0.0073$, recall $0.1453 \pm 0.0139$, and loss $6.5549 \pm 0.0618$. This delivers a $+0.0399$ (+25.2%) mAP50 improvement over the submitted stretch baseline ($0.1586 \pm 0.0104$) and improves every fold individually (0.2004, 0.2036, 0.1914). Mean AP50: Spaghetti $0.0336 \pm 0.0030$, Layer cracking $0.1510 \pm 0.0272$, Over extrusion $0.3432 \pm 0.0354$, Stringing $0.0884 \pm 0.0258$, Warping $0.3763 \pm 0.0367$.
  - Custom Faster R-CNN (scale `s`): selected epochs `28` / `42` / `50` with aggregate mAP50 $0.3280 \pm 0.0282$, mAP50-95 $0.1198 \pm 0.0097$, precision $0.2985 \pm 0.1086$, and recall $0.1167 \pm 0.0183$. This matches parity with the submitted scheduled scratch baseline ($0.3258 \pm 0.0275$ mAP50) across folds (0.3203, 0.3592, 0.3043). Mean AP50: Spaghetti $0.0434 \pm 0.0049$, Layer cracking $0.2694 \pm 0.0506$, Over extrusion $0.5533 \pm 0.0920$, Stringing $0.2352 \pm 0.0462$, Warping $0.5384 \pm 0.0796$.
  - Practical decision: Letterbox geometry is formally adopted for Phase 2+ practical investigations, standardizing input geometry with Ultralytics conventions while leaving submitted strict-scratch baselines frozen.
