...
>
> **Last updated:** 2026-07-21 — a YOLO26 inference-branch diagnostic and a separately pre-registered ImageNet-initialized Faster R-CNN transfer study are prepared; all completed scratch and VOC benchmark protocols remain closed.
...
- **Faster R-CNN status:** the initial 960-pixel three-fold Faster R-CNN run is invalid as a benchmark because it predates an RPN positive-supervision repair. Its mAP50 $0.0075 \pm 0.0079$ is retained only as a diagnostic of the defect. The repaired replacement baseline is valid: mAP50 $0.0935 \pm 0.0522$, mAP50-95 $0.0284 \pm 0.0203$, precision $0.1441 \pm 0.0157$, and recall $0.1445 \pm 0.0369$. It has lower aggregate mAP than direct YOLO26 + NMS (`0.1127` / `0.0343`) and much larger variance, but higher recall (`0.1445` versus `0.0588`). Retain it as the valid scratch two-stage reference; retain direct YOLO26 + NMS as the selected scratch-detector configuration.
- **Next required action:** do not reopen completed scratch YOLO26 or scratch Faster R-CNN DFL, loss, sampler, class-weight, augmentation, threshold, duration, resolution, anchor, or postprocessing sweeps. One distinct practical Faster R-CNN transfer-learning study is pre-registered below: ImageNet backbone initialization plus a lower backbone learning rate, with all other candidate-fold settings frozen. Retain direct 960 + class-aware-NMS YOLO26 as the current measured scratch configuration until the one-to-many branch diagnostic completes across all folds.
...
### Training-system diagnostics and practical transfer study (2026-07-21)
...
- **YOLO26 inference-branch diagnostic:** the local architecture trains a full-gradient one-to-many branch and a detached-feature one-to-one branch, but historical inference decodes only the one-to-one branch. A no-retraining validation diagnostic with identical class-aware NMS settings showed the one-to-many branch is stronger on two independent validation contexts: VOC 2007 `val` mAP50 `0.1657` / mAP50-95 `0.0683` / recall `0.2191` versus one-to-one `0.1452` / `0.0569` / `0.0774`; 3D-print candidate fold 1 mAP50 `0.1293` / mAP50-95 `0.0453` / recall `0.0888` versus one-to-one `0.1107` / `0.0326` / `0.0412`. `eval_yolo26.py` and its K-fold wrapper now expose `--inference-branch one2one|one2many`, defaulting to legacy `one2one` so historical reports remain reproducible. A complete one-to-many three-fold candidate validation evaluation was started but interrupted during slow metric aggregation; it has not yet established a new default or changed any reported result.
- **Pre-registered practical Faster R-CNN transfer study:** `models/faster_rcnn.py` now supports architecture-compatible ImageNet initialization through torchvision: local `s` maps to ResNet-18 and local `m` maps to ResNet-34; local `l` is intentionally rejected because its wider channel shapes are incompatible. Mapping tests, actual ResNet-18 ImageNet weight download, optimizer parameter groups, trainer dry run, generic runner dry run, and all existing model tests passed. The single permitted candidate transfer study changes only backbone initialization from random to ImageNet and applies a fixed `0.1` backbone learning-rate multiplier; FPN, RPN, heads, data folds, 960 input, batch 2, 50 epochs, seed 42, AdamW base LR `1e-4`, weight decay `5e-4`, positive-weight power `0.25`, no sampler, no augmentation, NMS, and project metrics remain frozen. This is a practical transfer-learning comparison, not a scratch architecture-only comparison with YOLO26 or YOLO11n.
- **Transfer boundary:** run the ImageNet-initialized Faster R-CNN study once under a distinct run root and evaluate its three validation folds once. Do not use the contaminated public candidate test split, tune from intermediate folds, or add a second transfer variation. The first invocation downloads torchvision's ResNet-18 ImageNet weights into the local Torch cache if they are absent.
...
- Faster R-CNN has a completed repaired group-disjoint scratch baseline. It remains an unpretrained two-stage reference, not a selected replacement for the custom YOLO26 or pretrained YOLO11n results.
