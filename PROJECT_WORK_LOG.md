...
>
> **Last updated:** 2026-07-19 — a Faster R-CNN RPN-supervision defect was isolated and repaired through a same-images overfit diagnostic; the pre-fix Faster R-CNN CV result is invalidated and 960 + class-aware NMS remains the best measured custom YOLO26 configuration.
...
- **Faster R-CNN status:** the initial 960-pixel three-fold Faster R-CNN run is invalid as a benchmark because it predates an RPN positive-supervision repair. Its mAP50 $0.0075 \pm 0.0079$ is retained only as a diagnostic of the defect and must not be compared with the custom YOLO26 or pretrained YOLO11n results. The repaired code passed a deliberately leaked same-images overfit diagnostic; a replacement group-disjoint baseline may be run once under the same frozen fold protocol and a new run root.
...
- **Next Faster R-CNN boundary:** commit/push the RPN repair and pull it onto the training PC. Then, if the comparison remains in scope, run exactly one replacement 960-pixel, three-fold Faster R-CNN baseline under the same seed, batch size, optimizer, weights, NMS, and project metrics as the invalidated run, using a new run root. Do not compare either scratch Faster R-CNN result directly with pretrained YOLO11n as an architecture-only contest; disclose the Faster R-CNN `s`/YOLO26 `n` capacity difference and YOLO11n pretraining advantage.
