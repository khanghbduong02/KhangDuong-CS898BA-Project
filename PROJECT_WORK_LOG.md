...
>
> **Last updated:** 2026-07-23 — strict-scratch mAP50 checkpoint selection completed for both local architectures; YOLO26 horizontal-flip TTA was rejected; a shared warmup-plus-cosine schedule is implemented and smoke-tested for one new strict-scratch CV study; no candidate public-test data was used.
...
- **Experiment boundary:** do not reopen completed scratch YOLO26 or scratch Faster R-CNN DFL, loss, sampler, class-weight, augmentation, threshold, duration, resolution, anchor, postprocessing, P2, plateau-controller, or combined architecture/controller sweeps. The ImageNet-transfer, P2/controller, and strict-scratch checkpoint-selection studies are complete. The only newly user-authorized treatments are the fixed Faster R-CNN horizontal-flip diagnostic and the isolated warmup-plus-cosine schedule study described below; do not use the contaminated candidate test split or add another variation on this export. The one-to-many branch correction is a completed no-retraining inference selection, not a new training sweep.
...
- **User-authorized horizontal-flip test-time augmentation: YOLO26 completed and rejected; Faster R-CNN pending.** `inference_tta.py` supplies a shared horizontal-flip/coordinate-unflip/merge path for both local evaluators. `--tta-hflip` runs a frozen checkpoint on each original validation image and its horizontal flip, maps flip predictions back to original pixel coordinates, then applies the existing class-aware NMS thresholds and detection limit. It is off by default, preserves trained weights and source data, forbids YOLO legacy-top-k mode, and writes distinct `_tta_hflip_` outputs so standard artifacts cannot be overwritten. Synthetic coordinate restoration, duplicate suppression, wrapper dry runs, evaluator smoke coverage, and normal model regressions passed. The fixed local YOLO26 mAP-selected checkpoints were then evaluated once: mAP50 fell from $0.1586 \pm 0.0104$ to $0.1409 \pm 0.0089$, mAP50-95 from $0.0512 \pm 0.0043$ to $0.0475 \pm 0.0044$, and precision from $0.1106 \pm 0.0060$ to $0.0828 \pm 0.0046$, despite recall rising from $0.1306 \pm 0.0067$ to $0.1741 \pm 0.0093$. AP50 fell for Layer cracking, Over extrusion, Stringing, and Warping; therefore horizontal-flip TTA is rejected for YOLO26. The one remaining fixed evaluation is the same diagnostic on the existing local Faster R-CNN mAP-selected checkpoints, which reside on the other PC; it must use grouped `valid` folds only and no retraining or candidate-test data.
- **User-authorized warmup-plus-cosine schedule: prepared, not yet trained.** `training_control.py` now provides a deterministic epoch-based scheduler shared by both trainers. Historical constant learning rate remains the default. The new `--lr-schedule cosine` treatment uses three linear warmup epochs from $0.1\times$ to $1.0\times$ the base rate, then cosine decay to $0.02\times$ the base rate at epoch 50. It is deliberately independent of validation metrics and requires the existing plateau/early-stopping controller to be disabled, isolating the optimization-policy change while leaving mAP50 checkpoint selection unchanged. The K-fold runners forward, record, and validate schedule settings; checkpoints record schedule state, and Faster R-CNN history records the epoch factor. Formula/compatibility tests, full model regressions, exact-runner dry runs, and direct five-epoch CUDA smokes passed for both architectures; the smokes confirmed factors $0.10$, $0.55$, $1.00$, $1.00$, and $0.02$ across the relevant epochs. The planned CV reproduces the current 960-pixel mAP-selection protocols with random initialization, P2 disabled, the same data/folds/losses/class weights/NMS/seed, 50 maximum epochs, and no candidate-test use. Fresh run roots are mandatory. This is a single schedule-only study, not a promise of pretrained YOLO11 parity.
- **Final demonstration support:** `demo_ultralytics.py` creates annotated batch-inference images plus a JSON prediction summary from a frozen Ultralytics checkpoint. It is explicitly a visual demonstration tool rather than a new metric-selection path; the documented example uses grouped validation images and continues to exclude the public candidate test split. A two-image smoke demonstration passed with the frozen YOLO11n fold-1 checkpoint: it saved annotated JPEG outputs and `demo_predictions.json`, including a visually verified Spaghetti detection. The same script should use the fine-tuned YOLO26n fold-1 `best.pt` for the final recorded batch demonstration.
...
The legacy custom evaluator selected a global top-$k$ set from decoded predictions but did not suppress overlapping same-class boxes. A new [class_aware_nms()](models/yolo26_torch.py) decoder now operates on all raw decoded one-to-one candidates **before** legacy top-$k$ truncation. It uses `torchvision.ops.batched_nms`, retains scores at least `0.001` for AP, applies class-aware IoU suppression at `0.70`, and keeps at most 300 detections per image. [test.py](test.py) now verifies that duplicate same-class boxes are suppressed while an overlapping different-class detection remains. `torchvision` is explicitly listed in [requirements.txt](requirements.txt).
...
- [test_faster_rcnn.py](test_faster_rcnn.py): Faster R-CNN FPN/RPN target-assignment and model-behavior regression tests.
- [test_training_control.py](test_training_control.py): scheduler, early-stopping, and early-stopped K-fold completion regression tests.
