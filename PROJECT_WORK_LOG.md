...
>
> **Last updated:** 2026-07-23 — strict-scratch mAP50 checkpoint selection completed for both local architectures; horizontal-flip test-time augmentation was rejected for YOLO26 and is ready for one fixed Faster R-CNN validation-only check; no candidate public-test data was used.
...
- **Experiment boundary:** do not reopen completed scratch YOLO26 or scratch Faster R-CNN DFL, loss, sampler, class-weight, augmentation, threshold, duration, resolution, anchor, postprocessing, P2, scheduler, or combined architecture/controller sweeps. The ImageNet-transfer, P2/controller, and strict-scratch checkpoint-selection studies are complete. The only newly user-authorized treatment is the fixed horizontal-flip test-time-augmentation diagnostic described below; do not use the contaminated candidate test split or add another variation on this export. The one-to-many branch correction is a completed no-retraining inference selection, not a new training sweep.
