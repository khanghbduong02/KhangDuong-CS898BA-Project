...
>
> **Last updated:** 2026-07-23 — the same-fold pretrained official YOLO26n practical reference nearly matches the pretrained YOLO11n reference; a strict-scratch mAP50 checkpoint-selection intervention is implemented and smoke tested but not yet trained; no candidate public-test data was used.
...
- **Experiment boundary:** do not reopen completed scratch YOLO26 or scratch Faster R-CNN DFL, loss, sampler, class-weight, augmentation, threshold, duration, resolution, anchor, postprocessing, P2, scheduler, or combined architecture/controller sweeps. The distinct ImageNet-transfer and P2/controller treatments have completed their single permitted training/evaluation cycles; do not use the contaminated candidate test split or add another variation on this export. The sole user-authorized exception is the strict-scratch mAP50 checkpoint-selection study documented below. The one-to-many branch correction is a completed no-retraining inference selection, not a new training sweep.
...
- **Strict-scratch mAP50 checkpoint-selection intervention: prepared, not yet trained.** Both local trainers now accept `--checkpoint-selection map50`. After each epoch, they decode their normal inference path on the validation fold and compute the project mAP50; that value selects `best.pt`. Validation loss remains the only `ReduceLROnPlateau`/early-stopping signal, so checkpoint selection does not alter the requested patience formula. Unit tests, model tests, direct one-epoch tiny-fold CUDA smokes, checkpoint metadata inspection, and generic K-fold dry runs passed for both architectures. The pre-registered full-CV protocol reproduces the respective historical scratch 960 baselines except for this selection criterion: no pretrained weights, P2 disabled, plateau controller disabled, focal gamma `2`/positive-weight power `0.25` for YOLO26, positive-weight power `0.25` for Faster R-CNN, seed `42`, 50 maximum epochs, and the same existing NMS/project metrics. Fresh run roots are mandatory. This can improve metric-aligned checkpoint choice but is not expected to close the full COCO-pretraining gap by itself.
