...
>
> **Last updated:** 2026-08-24 — shared post-submission letterbox geometry was implemented and smoke-validated on the frozen dataset; no submitted baseline, dataset, checkpoint, or experimental result was changed.
...
- **Post-submission practical study:** future transfer-learning and Ultralytics-style engineering work is separate from the submitted strict-scratch study. [POST_SUBMISSION_PRACTICAL_PLAN.md](POST_SUBMISSION_PRACTICAL_PLAN.md) now freezes the current dataset, labels, grouped folds, class taxonomy, preprocessing inputs, and candidate public-test boundary. Its active order is shared letterbox geometry, controlled Ultralytics-style augmentation, parity evaluation with the existing pretrained references, then documented transfer learning. All resulting metrics remain selection-aligned grouped-development evidence, not independent generalization claims. No new training has begun.
- **Post-submission shared letterbox geometry: implemented and smoke-validated.** [image_geometry.py](image_geometry.py) provides a reversible source-to-model transform for historical `stretch` and new aspect-ratio-preserving `letterbox` modes. Both local training datasets, one-fold evaluators, K-fold train/evaluation wrappers, and custom demo tools now accept checkpoint-aware resize settings; `stretch` is the legacy default, newly trained checkpoints save `resize_mode`, and old checkpoints resolve safely to `stretch`. Synthetic compatibility/round-trip/dataset/demo/runner tests passed. One-epoch CUDA letterbox smokes on the unchanged fold-1 data completed for YOLO26 (`22` train / `11` valid images) and Faster R-CNN (`4` train / `2` valid images); both evaluators restored `resize_mode=letterbox` from checkpoint metadata. Their zero AP values are intentionally non-reportable smoke results because the subsets are tiny and not class-representative. K-fold train and evaluation dry runs confirmed `--resize-mode letterbox` propagation and distinct override metric paths. No full letterbox CV result exists yet.
