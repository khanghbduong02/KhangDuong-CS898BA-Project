...
The completed strict-scratch mAP50-selection study supersedes that validation-loss-selected reference for this frozen candidate protocol. It kept the random `s` backbone, P3--P6 FPN, 960-pixel grouped folds, loss, optimizer, class-weight power, seed, and per-class NMS unchanged; only the checkpoint criterion changed. The selected epochs were `50` / `47` / `35`, producing mAP50 $0.3191 \pm 0.0164$, mAP50-95 $0.1145 \pm 0.0099$, precision $0.1777 \pm 0.0329$, and recall $0.1271 \pm 0.0201$. Thus it improves mAP50 by $0.2256$, mAP50-95 by $0.0861$, and precision by $0.0336$ relative to the repaired validation-loss baseline, while recall falls by $0.0174$. It remains a same-fold checkpoint-selection result rather than an independent held-out test estimate; no candidate public-test images were used.

Both custom trainers now share validation-loss `ReduceLROnPlateau` and early stopping. The default policy is reduction patience `8`, factor `0.5`, cooldown `2`, minimum LR `1e-6`, and early-stopping patience `18`, satisfying $18 = 2(8) + 2$. A real LR reduction resets the early-stopping counter so the reduced rate can improve before a run stops. Set both patience arguments and cooldown to `0` for an explicit constant-LR reproduction or an isolated epoch-based schedule study. Checkpoints record the controller state, current learning rates, completion state, and stop reason; K-fold runners accept a matching early-stopped fold as complete. Both architectures also offer `--use-p2`: YOLO26 adds a stride-4 PAN detection head, while Faster R-CNN expands from P3--P6 to P2--P6 with 16-pixel base anchors and stride-aware RoI assignment. The flag changes checkpoint shapes and is disabled by default for historical compatibility.

An optional epoch-based schedule is also available: `--lr-schedule cosine --warmup-epochs 3 --warmup-start-factor 0.1 --cosine-final-factor 0.02`. It ramps from 10% to 100% of the configured base LR over the first three epochs, then decays deterministically to 2% at the final epoch. The default `constant` schedule exactly preserves historical training. Epoch-based scheduling is intentionally mutually exclusive with validation-loss plateau/early stopping, so a schedule-only study must disable both controls; mAP50 checkpoint selection remains unchanged.

The local trainers also accept `--checkpoint-selection map50`. This computes project validation mAP50 after each epoch and uses it to save `best.pt`; `val_loss` still exclusively controls `ReduceLROnPlateau` and early stopping. The historical default, `--checkpoint-selection val_loss`, remains available for exact reproduction. The completed pre-registered strict-scratch study preserved the historical model topology, data folds, losses, resolution, seed, and 50-epoch budgets while disabling the newer plateau controller. For YOLO26, it selected epochs `48` / `47` / `49` and improved the one-to-many 960 result to mAP50 $0.1586 \pm 0.0104$, mAP50-95 $0.0512 \pm 0.0043$, precision $0.1106 \pm 0.0060$, and recall $0.1306 \pm 0.0067$. This is $+0.0316$ mAP50 and $+0.0092$ mAP50-95 over validation-loss selection, with a small precision decrease and recall increase. Each fold uses its validation data both for epoch selection and reporting, so these values are selection-aligned grouped-CV estimates rather than independent test results.

Both local trainers also support optional model exponential moving average through `--ema-decay`; the default `0` preserves raw-weight training exactly. For a nonzero value, each optimizer step updates a shadow state that averages floating parameters and BatchNorm running statistics, while retaining current non-floating counters. The requested decay is normalized by the number of batches per epoch so its epoch-scale memory is comparable between the models' different batch sizes. The completed strict-scratch study used per-epoch retention `0.9`, then used EMA weights for validation mAP50 selection and `best.pt` inference while raw validation loss continued to govern the plateau controller. It was rejected for both systems: YOLO26 mAP50 fell from $0.1586 \pm 0.0104$ to $0.0507 \pm 0.0154$, and the cosine-scheduled Faster R-CNN mAP50 fell from $0.3258 \pm 0.0275$ to $0.3166 \pm 0.0322$. The option remains for reproduction only; selected local configurations use raw weights.

The local trainers also offer `--online-augmentation photometric`; the default `none` reproduces raw-image training exactly. The single fixed photometric policy runs only on in-memory training tensors after resize: brightness, per-channel contrast, and gamma are independently sampled from $[0.9, 1.1]$, then Gaussian noise with standard deviation $0.01$ is added. It has no geometric transform, class rebalance, stored-file generation, or label/box change; validation images and labels remain untouched. This is distinct from the rejected offline minority augmentation treatment, which created transformed examples and changed class counts. The completed study was not adopted as an overall replacement: YOLO26 aggregate mAP50 was effectively unchanged ($0.1586$ to $0.1588$) with Layer cracking gains but Warping/Stringing AP50 losses, while Faster R-CNN mAP50 fell from $0.3258$ to $0.2999$.
...
Both local evaluators also support optional `--tta-hflip` inference. It runs each existing checkpoint on the original validation image and a horizontal flip, maps flipped boxes back to the original coordinate system, then merges them with the same class-aware NMS policy. It is disabled by default, leaves training and source data unchanged, and is evaluated only on grouped validation folds; it is not a candidate-test path. The completed diagnostic was rejected for both models: YOLO26 mAP50 fell from $0.1586$ to $0.1409$, and Faster R-CNN mAP50 fell from $0.3191$ to $0.3075$. The option remains only for reproducibility.
...
The completed mAP50-based checkpoint-selection intervention improved both historical 960 protocols without modifying the dataset, labels, folds, image-processing variants, model initialization, loss, optimizer, augmentation, or inference settings. A subsequent isolated warmup-plus-cosine schedule was rejected for YOLO26: mAP50 fell from $0.1586 \pm 0.0104$ to $0.0947 \pm 0.0051$. For Faster R-CNN, it produced a modest mAP50 increase from $0.3191 \pm 0.0164$ to $0.3258 \pm 0.0275$, a mAP50-95 increase from $0.1145 \pm 0.0099$ to $0.1239 \pm 0.0111$, and substantially higher fixed-threshold precision, at lower recall. The final EMA-only attempt was also rejected: it reduced YOLO26 mAP50 to $0.0507 \pm 0.0154$ and Faster R-CNN mAP50 to $0.3166 \pm 0.0322$. The final online photometric policy was mixed/neutral for YOLO26 ($0.1588 \pm 0.0103$ mAP50) and negative for Faster R-CNN ($0.2999 \pm 0.0234$ mAP50). Fixed-LR raw YOLO26 and raw-weight scheduled Faster R-CNN therefore remain the current local mAP-oriented configurations. All values remain selection-aligned grouped-validation estimates rather than independent test results. No candidate public-test evaluation or further unregistered sweep is permitted.
