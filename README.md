...
The local trainers also accept `--checkpoint-selection map50`. This computes project validation mAP50 after each epoch and uses it to save `best.pt`; `val_loss` still exclusively controls `ReduceLROnPlateau` and early stopping. The historical default, `--checkpoint-selection val_loss`, remains available for exact reproduction. A pre-registered strict-scratch study is prepared to test mAP50 selection only, with the historical model topology, data folds, losses, resolution, seed, and 50-epoch budgets preserved and the newer plateau controller disabled.
...
The only additional local scratch intervention currently authorized is mAP50-based checkpoint selection, applied separately to the historical YOLO26 and Faster R-CNN 960 protocols. It does not modify the dataset, labels, folds, image-processing variants, model initialization, loss, optimizer, augmentation, or inference settings. It is expected to improve alignment between the selected checkpoint and the reported metric, but it cannot by itself guarantee parity with pretrained YOLO11n.
