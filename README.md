...
The completed mAP50-based checkpoint-selection intervention improved both historical 960 protocols without modifying the dataset, labels, folds, image-processing variants, model initialization, loss, optimizer, augmentation, or inference settings. Faster R-CNN now reaches 66.5% of the pretrained YOLO11n reference mAP50 but remains below its $0.4799 \pm 0.0262$ value. The next registered strict-scratch training intervention is a warmup-plus-cosine schedule only: it retains the current mAP50 checkpoint selection, random initialization, 960-pixel input, grouped folds, losses, and inference policy, while disabling the plateau controller to isolate the schedule. No candidate public-test evaluation is permitted.
