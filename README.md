...
Both local trainers also support optional model exponential moving average through `--ema-decay`; the default `0` preserves raw-weight training exactly. For a nonzero value, each optimizer step updates a shadow state that averages floating parameters and BatchNorm running statistics, while retaining current non-floating counters. The requested decay is normalized by the number of batches per epoch so its epoch-scale memory is comparable between the models' different batch sizes. The prepared strict-scratch study fixes the per-epoch retention at `0.9`, which gives the EMA an approximately ten-epoch smoothing horizon without retaining a material contribution from random initialization at epoch 50. EMA weights—not raw weights—then compute validation mAP50 and populate `best.pt`, while raw validation loss continues to govern the plateau controller. The selected checkpoint records `checkpoint_weights=ema`; the raw `last.pt` records EMA metadata without duplicating the full shadow tensor state because trainer resume is intentionally unsupported. The full EMA-only grouped-CV study is prepared but not yet trained.
...
The completed mAP50-based checkpoint-selection intervention improved both historical 960 protocols without modifying the dataset, labels, folds, image-processing variants, model initialization, loss, optimizer, augmentation, or inference settings. A subsequent isolated warmup-plus-cosine schedule was rejected for YOLO26: mAP50 fell from $0.1586 \pm 0.0104$ to $0.0947 \pm 0.0051$. For Faster R-CNN, it produced a modest mAP50 increase from $0.3191 \pm 0.0164$ to $0.3258 \pm 0.0275$, a mAP50-95 increase from $0.1145 \pm 0.0099$ to $0.1239 \pm 0.0111$, and substantially higher fixed-threshold precision, at lower recall. Fixed-LR YOLO26 and scheduled Faster R-CNN are therefore the current local mAP-oriented configurations. The only new strict-scratch intervention prepared after those decisions is EMA weight averaging; all values remain selection-aligned grouped-validation estimates rather than independent test results. No candidate public-test evaluation or further unregistered sweep is permitted.
